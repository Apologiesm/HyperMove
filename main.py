...
class LiquidSpeedGraph(QWidget):
    ...
    def update_data(self, val):
        ...
        self.max_val = max(max(self.points), 1024*1024*10) 
    ...
    def paintEvent(self, event):
        ...
        grad_back.setColorAt(1, QColor(0, 243, 255, 0))
        ...
        grad_front.setColorAt(1, QColor(0, 243, 255, 0))
        painter.setBrush(QBrush(grad_front))
        painter.setPen(QPen(QColor(0, 243, 255, 255), 2))
...
class AnimatedSmartDropZone(QFrame):
    ...
    def __init__(self, title, is_source=True):
        ...
        if is_source:
            self.btn_file = self._create_btn("📄 File")
            self.btn_file.clicked.connect(lambda: self.browse_clicked.emit('file'))
    ...
    def crossfade_view(self, show_selected):
        ...
        a1 = QPropertyAnimation(self.eff_empty, b"opacity")
        a1.setDuration(300); a1.setEndValue(0.0 if show_selected else 1.0)
        a2 = QPropertyAnimation(self.eff_sel, b"opacity")
        a2.setDuration(300); a2.setEndValue(1.0 if show_selected else 0.0)
        self.anim_group.addAnimation(a1); self.anim_group.addAnimation(a2); self.anim_group.start()
        self.view_empty.setAttribute(Qt.WidgetAttribute.WA_TransparentForMouseEvents, show_selected)
    ...
    def set_path(self, path):
        ...
        try:
            drive_letter = p.drive if p.drive else ""
            if self.is_source: self.lbl_sel_info.setText(f"{drive_letter} {format_size(p.stat().st_size if p.is_file() else sum(f.stat().st_size for f in p.rglob('*') if f.is_file()))}")
            else: self.lbl_sel_info.setText(f"{drive_letter} Drive Free: {format_size(shutil.disk_usage(path).free)}")
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.engine.signals.progress_update.connect(self.update_progress)
